...
export default function Post() {
  ...
  const handleLikeDislike = async (type: "like" | "dislike", mutation: any) => {
    if (!token || !userData) {
      setOpenLogin(true);
      return;
    }
          ...
          <div className="w-full flex flex-col gap-4">
            ...
            <div className="flex items-center justify-end gap-4">
              <button
                ...
                onClick={() => handleLikeDislike("like", updateBlogLike)}
              >
                <span>{post.like_count || 0}</span>
                {isPostLiked ? <BiSolidLike size={22} /> : <BiLike size={22} />}
              </button>

              <button
                className={`flex items-center gap-2 text-gray-400`}
                onClick={() => handleLikeDislike("dislike", dislikeBlog)}
              >
                {isPostDisliked ? (
            ...
            <div>
              <div>
                <span className="text-xl font-normal mr-2">Comments</span>
