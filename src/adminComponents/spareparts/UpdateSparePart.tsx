...
} from "../../features/spare-parts/spare-parts";
...
export default function UpdateSpareParts() {
  const { spareId } = useParams<{ spareId: string }>();
  const { data: sparePart } = useGetSpareByIdQuery(spareId!);
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    setValue,
    control,
  } = useForm<SpareFormInputs>({ defaultValues: sparePart });
  ...
  useEffect(() => {
    if (sparePart) {
      (Object.keys(sparePart) as (keyof SpareFormInputs)[]).forEach((key) => {
        setValue(key, sparePart[key]);
      });
      setSelectedImage(sparePart.cover_image || null);
      const imagePaths = sparePart.images?.map((img: string) => img) || [];
  ...
  const onSubmit: SubmitHandler<SpareFormInputs> = async (formData) => {
    ...
    try {
      await updateSpare({ id: spareId!, spareData: formData }).unwrap();
