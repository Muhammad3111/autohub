...
export default function UpdateCar() {
  ...
  const { data: carData, isLoading } = useGetCarByIdQuery(carId!);
  ...
  useEffect(() => {
    if (carData) {
      (Object.keys(carData) as (keyof CarFormInputs)[]).forEach((key) => {
        setValue(key, carData[key]);
      });
      setSelectedImage(carData.cover_image?.path || null);

      // Imagesni path bo'yicha string massivga aylantirish
      const imagePaths =
        carData.images?.map((image: { path: string }) => image.path) || [];
  ...
  const onSubmit: SubmitHandler<CarFormInputs> = async (data) => {
    ...
    try {
      await updateCar({ id: carId!, carData: data });
      toast.success("Avtomobil muvaffaqiyatli yangilandi");
    } catch (error) {
      console.error(error);
      toast.error("Avtomobilni yangilashda xatolik yuz berdi");
    }
  };
  ...
  return (
    <div className="flex flex-col gap-4 p-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-2xl text-black">Avtomobilni Yangilash</h1>
        <Button
          path="/admin/cars"
      ...
      >
        ...
        <div className="col-span-1 flex flex-col gap-5">
          ...
          <KeyValueInputs
            name="properties"
            register={register}
            defaultFields={Object.entries(carData.properties || {}).map(
              ([key, value]) => ({
                key_uz: key,
                value_uz: value as string, // `value`ni `string` deb aniqlash
              })
            )}
          />
        </div>
