...
import { collection, priceButtons } from "../../mock/data.json";
...
export default function VehicleFilter() {
  ...
  const pricesButtons: PriceButton[] = priceButtons;
  return (
          ...
          {pricesButtons.map((p) => (
            <button
              ...
              className={`text-base border px-2 py-1 ${
                p.id === price.id ? "bg-primary text-white" : "bg-transparent"
              } duration-150`}
            >
              {p.end === 0
                ? p.start === 0
          ...
          {collection.map((c) => (
            <button
              ...
              className={`text-base border uppercase px-2 py-1 ${
                c.title === model ? "bg-primary text-white" : "bg-transparent"
              }`}
            >
