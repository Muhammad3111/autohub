...
export default function BrandsFilter() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const brandItems = brandListRef.current.querySelectorAll("[data-letter]");

      for (let i = 0; i < brandItems.length; i++) {
        const item = brandItems[i] as HTMLElement;
        if (item.offsetTop - brandListRef.current.offsetTop > scrollPosition) {
    ...
    return () => {
      if (brandListRef.current) {
        brandListRef.current.removeEventListener("scroll", handleScroll);
      }
    };
  }, []);
  return (
        ...
        <div className="flex flex-col w-full relative">
          ...
          >
            {sortedBradns.map((brand, index) => {
              const prevLetter = index > 0 ? brands[index - 1].name[0] : "";
              const currentLetter = brand.name[0];
              const showLetterHeader = prevLetter !== currentLetter;

              return (
                <div key={brand.id} data-letter={currentLetter} className="">
                  {showLetterHeader && (
